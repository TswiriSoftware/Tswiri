...
class NavigatorPainterIsolate extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    PainterData painterData = PainterData.fromJson(jsonDecode(message));
    log(painterData.toString());
    ...
    canvas.drawCircle(
        ...
        painterData.finderCircleRadius,
        paintSimple(
            color: Colors.black,
            strokeWidth: 2.0,
            style: PaintingStyle.stroke));

    for (var painterBarcode in painterData.painterBarcodeData) {
      //Draw borders.
      canvas.drawPoints(PointMode.polygon, painterBarcode.offsets(),
    ...
    // //Rotate the canvas.
    canvas.rotate(painterData.offsetToBarcodeAngle);
    //Translate the canvas back to original position
    canvas.translate(-screenCenter.dx, -screenCenter.dy);

    canvas.drawLine(screenCenter, Offset(0, 100), paintEasy(Colors.blue, 3.0));

    // //Draw the arrow
    // canvas.drawLine(painterData.arrowLineStart, painterData.arrowLineHead,
    //     paintEasy(Colors.blue, 3.0));

    // //ArrowHeadtop
    // Offset arrowHeadtop = Offset(
    //     painterData.arrowLineHead.dx - 30, painterData.arrowLineHead.dy + 20);

    // //ArrowHeadBottom
    // Offset arrowHeadbottom = Offset(
    //     painterData.arrowLineHead.dx - 30, painterData.arrowLineHead.dy - 20);
    // canvas.drawLine(

    //     painterData.arrowLineHead, arrowHeadtop, paintEasy(Colors.blue, 3.0));
    // canvas.drawLine(painterData.arrowLineHead, arrowHeadbottom,
    //     paintEasy(Colors.blue, 3.0));
