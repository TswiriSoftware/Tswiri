...
class NavigatorViewOLD extends StatefulWidget {
  const NavigatorViewOLD({
    Key? key,
  ...
  @override
  _NavigatorViewOLDState createState() => _NavigatorViewOLDState();
}

class _NavigatorViewOLDState extends State<NavigatorViewOLD> {
  bool isBusy = false;
  ...
  void initIsolate() async {
    ...
    mainPortImage1.listen((message) {
      if (message is SendPort) {
        ...
        isolateImagePort1!.send(['isar', isarDirectory!.path]);
        isolateImagePort1!.send(['write', 6]);
      } else if (message == 'configured') {
        setState(() {
          hasConfiguredIsolate1 = true;
          log('hasConfiguredIsolate1');
        });
      } else {
        drawImage(message);
      }
    ...
    mainPortImage2.listen((message) {
      if (message is SendPort) {
        ...
        isolateImagePort2!.send(['isar', isarDirectory!.path]);
        // await Future.delayed(const Duration(seconds: 1));
        isolateImagePort2!.send(['write', 8]);
      } else if (message == 'configured') {
        setState(() {
        ...
        body: Stack(
          children: [
            BarcodePositionScannerCameraView(
              ...
              customPaint: customPaint,
              onImage: (inputImage) {
                // log(isarDatabase!.containerEntrys
                //     .where()
                //     .findAllSync()
                //     .toString());
